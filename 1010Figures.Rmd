...
set_y_env(noise_type, is_monotone = F)
pX_unnorm <- build_pX_hat(m, fYtilde, fX, xlim=c(-10,10), normalize=F)
y_grid = seq(-75,75, length.out=600)
results2 = generate_ldens(y_grid,m_idx="PM",noise_type,r=25,n=150, N = 10^7, noise_sigma = noise_sigma)
...
ggsave(filename="./figures/f0m12n2.png", plot=p, width=853/96, height=278/96, dpi=96)
```
...
pX_unnorm <- Vectorize(mprime)
y_grid = seq(1e-8,150, length.out=500)
...
results2 = generate_ldens(y_grid,m_idx="hetero",noise_type="hetero",r=25, n=150, N = 10^6)
```
...
p=rbind(data1 %>% mutate(type="known m", m="m3 - Y"), data2 %>% mutate(type="PLR", m="m3 - Y"),
      data3 %>% mutate(type="known m", m="m3 - Z"), data4 %>% mutate(type="PLR", m="m3 - Z")) %>%
  ggplot() + geom_point(aes(x_vec,y_vec)) +
  geom_vline(data= xLR %>% pivot_longer(1:2, values_to = "xLR",names_to = "trType"),aes(xintercept = xLR, color="xL/xR"),linetype='dashed') +
  scale_colour_manual(values=c("red"), labels=c("xL,xR"))+
  labs(color="") +xlab("X")+ylab("Y or Z") + facet_grid(~m+type)+
...
ggsave(filename="./figures/f0m3.png", plot=p, width=853/96, height=278/96, dpi=96)
```
